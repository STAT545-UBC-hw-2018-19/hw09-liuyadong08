...
```{r load-data}
data1 <- read.delim("distance.tsv")
data2 <- read.delim("same_count.tsv")
```
...
![*Fig. 1* A barplot of the number of words that have the same or different starting letter and ending letter. ](barplot.png)

![*Fig. 2* A histogram of the interval between the starting letter and ending letter in English words ](distance.png)